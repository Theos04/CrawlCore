...
PATCH NOTES v2.5 — Enhanced Scene Detection & Object Tracking (merged with v2.4)
  ① Proper Places365 scene classification (ResNet50 trained on Places365) – with fallback
...
# ── Module 4F: Places365 Scene Classifier (v2.5) with graceful fallback ─────
class Places365Classifier:
    ...
    def load_model(self):
        ...
        try:
            ...
            # Set a longer timeout for torch.hub
            import torch.hub
            ...
            try:
                ...
                # Fallback: use a small built-in list (common scenes)
                self.labels = ['unknown'] * 365  # dummy
            ...
            print(f"    ⚠️  Places365 not available (using fallback scene detection). Reason: {e}")
            self.available = False
            return False
...
if __name__ == "__main__":
    ...
    # If --no-ml is set, we could optionally switch to a non‑ML analysis function.
    # For simplicity, we'll just proceed with the ML version; it will skip Places365 if not available.
