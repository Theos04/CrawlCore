"""
PATCH NOTES v2.4 — Ollama timeout fix for CPU inference:

  ① OLLAMA_TIMEOUT increased 30s → 120s  (CPU llama3 needs time)
  ② OLLAMA_RETRY_DELAY increased 2s → 10s (give model time to load between retries)
  ③ OLLAMA_MAX_RETRIES reduced 3 → 2      (faster fail if truly unavailable)
  ④ num_predict reduced 500 → 250         (faster response, still sufficient)
  ⑤ Pre-warm hint added in pipeline       (prints reminder to pre-warm model)

Previous patches:
  v2.3 — Complete overhaul with multi-language & multi-layer audio support:
...
class Config:
    ...
    # Ollama settings — v2.4 FIX: increased timeouts for CPU inference
    OLLAMA_URLS        = ["http://127.0.0.1:11434", "http://localhost:11434"]
    OLLAMA_MODEL       = "llama3"
    OLLAMA_ENABLED     = True
    OLLAMA_TIMEOUT     = 120     # ✅ FIX: was 30 — CPU llama3 needs 60-90s for 250 tokens
    OLLAMA_MAX_RETRIES = 2       # ✅ FIX: was 3 — fewer retries since each costs 120s
    OLLAMA_RETRY_DELAY = 10      # ✅ FIX: was 2 — give model time to finish loading
    OLLAMA_KEEP_ALIVE  = "10m"   # ✅ FIX: was 5m — keep model warm longer between videos
...
# ═══════════════════════════════════════════════════════════
# MODULE 2B — Enhanced audio analysis (multi-layer detection)
# ═══════════════════════════════════════════════════════════
def analyze_audio_enhanced(video_path: str) -> dict:
    """Enhanced audio analysis for multi-layered content (speech + music)."""
    ...
    try:
        ...
        # RMS energy for voice activity
        rms = librosa.feature.rms(y=y)[0]
        rms_mean = float(np.mean(rms))
        voice_activity = np.mean(rms > np.mean(rms) * 0.5) * 100
        
        # Voice confidence (lower spectral centroid usually indicates voice)
        voice_confidence = np.mean(spectral_centroids < 3000) * 100
        
        # Music confidence (presence of beats and harmonic structure)
...
# ═══════════════════════════════════════════════════════════
# MODULE 2C — Ollama content analysis (enhanced)
# ═══════════════════════════════════════════════════════════
...
def _ollama_warmup(working_url: str) -> bool:
    """
    Send a tiny prompt to ensure the model is loaded into memory.
    Returns True if model responds within timeout.
    This avoids cold-start timeouts on the real analysis prompt.
    """
    payload = json.dumps({
...
def _clean_json_response(response_text: str) -> str:
    ...
    v2.5: Stack-based extractor handles nested arrays/objects that regex misses.
    Handles llama3's common patterns: markdown fences, preamble text, trailing notes.
    """
    ...
    if fence_match:
        ...
        try:
            ...
            return candidate          # valid JSON inside fence → use it
        except Exception:
            pass                      # malformed → fall through to stack extractor

    # ── Step 2: stack-based extraction of outermost { ... } ──
    # Correctly handles nested objects and arrays that simple regex patterns miss.
    best = ""
...
def ollama_analyze_content_enhanced(transcript: str, ocr_text: str,
                                    transcript_quality: dict,
                                    audio_layers: list,
                                    duration_sec: float) -> dict:
    """
    Enhanced Ollama analysis with multi-language and audio layer awareness.
    v2.4: Added warm-up ping, increased timeout, reduced num_predict.
    """
    ...
    # ✅ FIX v2.4: Warm up model before sending the real prompt
    # This loads llama3 into RAM so the analysis call doesn't cold-start
    print(f"    🔥 Warming up {Config.OLLAMA_MODEL} (loading into RAM)...")
    warmup_ok = _ollama_warmup(working_url)
    if not warmup_ok:
        print(f"    ⚠️  Warm-up timed out — proceeding anyway (model may still load)")

    # Build context with audio layer info
    audio_context = ""
    if audio_layers:
    ...
    # Build text block with quality note
...
{{
  ...
  "confidence": 75
    ...
    payload = json.dumps({
        ...
        "options": {
            "temperature": 0.1,
            "num_predict": 250,   # ✅ FIX v2.4: was 500 — halved for faster CPU response
        },
        "keep_alive": Config.OLLAMA_KEEP_ALIVE
    }).encode("utf-8")
    ...
    for attempt in range(Config.OLLAMA_MAX_RETRIES):
        try:
            ...
            # ✅ FIX v2.5: Debug — show first 200 chars of what llama3 actually returned
            print(f"    📥 Raw response preview: {repr(response_text[:200])}")

            # Parse JSON — try clean extractor first, then fallback
            ...
            try:
                ...
                print(f"    📥 Cleaned text preview: {repr(cleaned_text[:200])}")
                import re
...
# ═══════════════════════════════════════════════════════════
# MODULE 3 — Librosa audio analysis (original kept for compatibility)
# ═══════════════════════════════════════════════════════════
def analyze_audio(video_path: str) -> dict:
    """Original audio analysis - now calls enhanced version."""
    return analyze_audio_enhanced(video_path)
...
<title>Reel Analysis Report v2.4</title>
...
<div class="header">
  <h1>🎬 Reel Intelligence Report <span style="font-size:.5em;color:#7c3aed;">v2.4</span></h1>
...
<div class="footer">Reel Analyzer v2.4 · Whisper · MediaPipe · EasyOCR · Ollama · Claude</div>
...
def analyze_video(video_path: str) -> dict:
    ...
    # ✅ FIX v2.4: Remind user to pre-warm Ollama before long pipeline
    ...
    print("\n🎵 [3/7] Enhanced audio analysis (multi-layer detection)...")
    audio_data = analyze_audio_enhanced(video_path)
    ...
    print(f"    Voice confidence: {audio_data.get('voice_confidence', 0)}%  "
          f"Music confidence: {audio_data.get('music_confidence', 0)}%")
    ...
    print("\n🤖 [6b] Ollama content analysis (enhanced)...")
    all_ocr = visual_summary.get("all_ocr_text", "")
    transcript = speech_data.get("transcript", "")
    
    ollama_result = ollama_analyze_content_enhanced(
        transcript, all_ocr,
    ...
    if ollama_result.get("available") and not ollama_result.get("error"):
        ...
        print(f"    Language: {ollama_result.get('content_language', '?')}  "
              f"Background music: {'✅' if ollama_result.get('has_background_music') else '❌'}")
        print(f"    📝 {ollama_result.get('content_summary', '')[:100]}…")
    else:
        print(f"    ⚠️  Ollama not available: {ollama_result.get('reason', 'unknown')}")
    ...
    print(f"  🎵 Audio:         {audio_data['tempo_bpm']} BPM / {audio_data['detected_key']}")
    print(f"  🔊 Audio Type:    {audio_data.get('primary_layer', 'unknown')}  "
          f"(speech: {audio_data.get('has_speech', False)}, "
          f"music: {audio_data.get('has_music', False)})")
    print(f"  ✂️  Pacing:        {visual_summary['pacing']} ({visual_summary['estimated_cuts']} cuts)")
    print(f"  👤 Face/Eye:      {int(visual_summary['face_presence_ratio']*100)}% / "
          f"{int(visual_summary['eye_contact_ratio']*100)}%")
    print(f"  🏠 Scene:         {visual_summary.get('indoor_outdoor', '?')} / "
          f"{visual_summary.get('dominant_setting', '?')}")
    print(f"  📝 Text:          {int(visual_summary['text_overlay_ratio']*100)}%  "
          f"Scripts: {visual_summary.get('scripts_detected', [])}")
    print(f"  🎙️  Transcript:    {speech_data.get('word_count', 0)} words  "
          f"Quality: {q.get('score', 0)}/100 ({'✅' if q.get('reliable') else '⚠️'})")
    
    if ollama_result.get("available") and not ollama_result.get("error"):
        print(f"  🤖 Content:       {ollama_result.get('topic_niche', '?')} · "
              f"{ollama_result.get('content_language', '?')} · "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Reel Analyzer v2.4")
    ...
    parser.add_argument("--ollama-timeout", type=int, default=Config.OLLAMA_TIMEOUT,
                        help=f"Ollama timeout in seconds (default: {Config.OLLAMA_TIMEOUT})")
    ...
    Config.OLLAMA_TIMEOUT  = args.ollama_timeout  # ✅ now configurable from CLI
