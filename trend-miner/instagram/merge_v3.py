"""
REEL ANALYZER v3.0 — ULTRA SMART EDITION
Fully merged from v2.5 + v3.0 patch notes.

WHAT'S NEW IN v3.0:
 ① PaddleOCR for stylized text (fallback to EasyOCR)
 ② YAMNet audio event detection (ghop, drinking, etc.)
 ③ CLIP meme matching (optional, off by default)
 ④ New niche: viral_meme with trending slang
 ⑤ Trend storage & pattern detection (JSONL)
 ⑥ All models gracefully degrade if not installed

CARRIED OVER FROM v2.5:
 ① Proper Places365 scene classification (ResNet50) – with fallback
 ...
 ⑥ Fixed Ollama timeout and retry logic (120s timeout, warm-up)
 ⑦ More accurate aesthetic scoring with rule-of-thirds
 ⑧ Enhanced audio analysis (multi-layer: speech, music, etc.)
...
try:
    ...
    print("⚠️  PaddleOCR not installed — pip install paddleocr")
...
class Config:
    ...
    TRENDS_LOG        = os.path.join(OUTPUT_DIR, "trends.jsonl")   # v3.0
    ...
    USE_PADDLE        = True
    USE_YAMNET        = True
    USE_CLIP          = False   # heavy — enable with --use-clip
...
# ══════════════════════════════════════════════════════════
# v3.0 — ENHANCED OCR (PaddleOCR → EasyOCR fallback)
# ══════════════════════════════════════════════════════════
def get_paddle_reader():
    global PADDLE_READER
    if PADDLE_READER is None and PADDLE_AVAILABLE:
...
def extract_text_enhanced(frame):
    """Try PaddleOCR first, then fallback to EasyOCR with preprocessing."""
    if Config.USE_PADDLE and PADDLE_AVAILABLE:
        try:
    ...
    return _fallback_easyocr_simple(frame)

def _fallback_easyocr_simple(frame):
    """Return (text, has_text) — simplified version for v3 frame loop."""
...
# ══════════════════════════════════════════════════════════
# v3.0 — YAMNET AUDIO EVENT DETECTION
# ══════════════════════════════════════════════════════════
...
def classify_audio_events(audio_path: str) -> list:
    ...
    try:
        ...
        for idx, cnt in zip(unique, counts):
            if cnt / total_frames > 0.1:
                events.append({
                    'class':      class_names[idx].decode(),
...
# ══════════════════════════════════════════════════════════
# MODULE 2B — AUDIO ANALYSIS (v3.0: +YAMNet events)
# ══════════════════════════════════════════════════════════
def analyze_audio(video_path: str) -> dict:
    cached = load_cache(video_path, "audio_enhanced_v3")
    if cached:
        print("  ♻️  Enhanced audio — using cache")
        return cached
    print("  🎵 Enhanced audio analysis (multi-layer + YAMNet)...")
    try:
    ...
    try:
        ...
        result = {
            ...
            "audio_events":         audio_events,   # v3.0
        }
    except Exception as e:
        print(f"  ❌ Audio analysis failed: {e}")
        result = _empty_audio()
    if os.path.exists(audio_path): os.remove(audio_path)
    save_cache(video_path, "audio_enhanced_v3", result)
    return result
...
# ══════════════════════════════════════════════════════════
# MODULE 4A — EASYOCR (full version, used in storyboard label)
# ══════════════════════════════════════════════════════════
...
def categorize_content(visual_summary: dict, audio_data: dict,
    ...
    audio_events= audio_data.get("audio_events",[])   # v3.0
    ...
    # v3.0: enrich combined_text with YAMNet event class names
    ...
    hook_patterns = {
        ...
        "meme_format":         ["ghop","m kha laadle","drinking sound","gulp","slurp","take this l"],  # v3.0
    }
    detected_hooks = [k for k,phrases in hook_patterns.items()
                      if any(p in combined_text for p in phrases)]

    niche_keywords = {
        "viral_meme":          ["ghop","m kha","laadle","drinking sound","gulp","slurp","take this l"],  # v3.0
    ...
    return {"detected_niche":detected_niche,"niche_scores":niche_scores,
...
# ══════════════════════════════════════════════════════════
# MODULE 4-FRAMES — FRAME ANALYSIS (v3.0: PaddleOCR + CLIP)
# ══════════════════════════════════════════════════════════
def analyze_frames_with_ml(path: str) -> tuple:
    ...
    with tqdm(total=total, desc="  🖼️  Frames (v3 ML+OCR)", unit="fr") as pbar:
        while cap.isOpened():
            ret,frame=cap.read()
            if not ret: break
            if frame_idx%interval==0:
                ...
                # v3.0 enhanced OCR: PaddleOCR → EasyOCR
                ocr_text_raw, has_text = extract_text_enhanced(frame)
                # Also get full easyocr detail for storyboard labels (words etc.)
                if EASYOCR_AVAILABLE:
                    easyocr_detail = extract_text_easyocr(frame)
                else:
                    easyocr_detail = {"words":[],"scripts_detected":[],"hindi_text":"","english_text":"","hook_text":ocr_text_raw,"readability_score":100 if has_text else 0}
                # Prefer PaddleOCR text but keep EasyOCR word-level detail
                ocr_text = ocr_text_raw if ocr_text_raw else easyocr_detail.get("text","")

                ml_analysis=classifier.analyze_image(frame,scene_data,face_data)

                # v3.0 CLIP meme (occasional)
                clip_meme=None
                ...
                frame_data.append({
                    ...
                    "motion_score":round(motion,3),"camera_move":camera_move,"scene_change":motion>20,
                    ...
                    "ocr_text":ocr_text,"ocr_word_count":len(ocr_text.split()),
                    "has_text_overlay":has_text or easyocr_detail.get("has_text",False),
                    "text_readability":easyocr_detail.get("readability_score",100 if has_text else 0),
                    "hook_text":easyocr_detail.get("hook_text",ocr_text[:150]),
...
def generate_ai_brief(full_report: dict) -> dict:
    ...
    compressed={
        ...
        "audio_events":[ev["class"] for ev in audio.get("audio_events",[])],  # v3.0
        "clip_meme":full_report.get("clip_meme",""),  # v3.0
    }
...
def generate_html_report(full_report: dict, ai_brief: dict, output_path: str):
    ...
    ml_stats=f"""<div class="card"><h2>🧠 ML Analysis</h2>
      ...
      {audio_events_html}{clip_meme_html}
    </div>"""
...
<title>Reel Analysis Report v3.0</title>
...
<div class="header">
  <h1>🎬 Reel Intelligence Report <span style="font-size:.5em;color:#7c3aed;">v3.0</span></h1>
  <p>Generated {datetime.now().strftime("%B %d, %Y at %H:%M")} · Whisper ({Config.WHISPER_MODEL}, {Config.WHISPER_LANG or "auto"}) · PaddleOCR+EasyOCR · MediaPipe · YOLO · Places365 · YAMNet · Ollama ({Config.OLLAMA_MODEL})</p>
...
<div class="footer">Reel Analyzer v3.0 ULTRA · Whisper · MediaPipe · PaddleOCR · EasyOCR · YOLO · Places365 · YAMNet · CLIP · Ollama · Claude</div>
<script>
...
def generate_storyboard(keyframes, frame_data, audio_data, category, output_path, cols=4):
    ...
    # v3.0: show audio events in header
...
# ══════════════════════════════════════════════════════════
# MODULE 11 — CSV LOGGING (v3.0: +audio_events, +clip_meme)
# ══════════════════════════════════════════════════════════
...
def analyze_video(video_path: str) -> dict:
    print(f"\n{'═'*62}")
    print(f"  🎬 Analyzing (v3.0 ULTRA): {os.path.basename(video_path)}")
    print(f"{'═'*62}\n")
    ...
    print("\n🎵 [3/7] Enhanced audio analysis (Librosa + YAMNet)...")
    ...
    print("\n🖼️  [4/7] Enhanced frame analysis (PaddleOCR + ML + CLIP)...")
    ...
    print("\n🎯 [7/7] Categorizing with v3.0 features...")
...
if __name__ == "__main__":
    parser=argparse.ArgumentParser(description="Reel Analyzer v3.0 ULTRA SMART (merged with v2.5)")
